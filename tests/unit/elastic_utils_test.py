...
class ElasticUtilsTest(unittest.TestCase):
  ...
  def test_record_slice_state(self):
    ...
    fake_recorder = Mock()
    fake_recorder.record_slice_state = Mock()
    
    elastic_utils.record_slice_state(fake_recorder)
    
    fake_recorder.record_slice_state.assert_called_once_with(
        available_slices=2,
        active_slices=1,
        total_slices=2
    )

    fake_recorder.record_slice_state.reset_mock()
  ...
  def test_record_elastic_event_start(self):
    ...
    fake_recorder.record_slice_state.assert_called_once()
